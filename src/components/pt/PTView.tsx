...
export const PTView: React.FC = () => {
  ...
  const handleSaveSheet = async () => {
    setIsSaving(true);
    try {
      await api.saveDailyPTSheet({ date, records: sheet });
      success('PT Attendance Saved', `Logged PT attendance for ${sheet.length} present personnel on ${date}`);
      fetchPT();
    } catch (err: any) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-white tracking-wide flex items-center gap-2">
            <Activity className="w-6 h-6 text-army-400" />
            Physical Training (PT) Attendance Management
          </h2>
        ...
        <div className="p-1 rounded-xl bg-tactical-900 border border-slate-700 flex items-center space-x-1 text-xs">
          ...
          >
            PT Attendance Analytics
          </button>
        </div>
      </div>

      {/* Tab 1: Daily PT Sheet */}
      {activeTab === 'daily' && (
              ...
              {canModify() && hasAppointment(['2IC', 'DUTY_OFFICER', 'DUTY_MUNSHI']) && (
                <>
                  ...
                  >
                    ...
                    <span>{isSaving ? 'Saving...' : 'Save PT Sheet'}</span>
                  </button>
                </>
              )}
            </div>
          ...
          {summary && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 rounded-xl bg-tactical-900 border border-slate-800 text-center">
                <span className="text-[10px] text-slate-400 font-bold uppercase">Daily PT Rate</span>
              ...
              <div className="p-3.5 rounded-xl bg-tactical-900 border border-slate-800 text-center">
                <span className="text-[10px] text-slate-400 font-bold uppercase">Excused from PT (Duty)</span>
                <div className="text-2xl font-black text-blue-400 font-mono mt-0.5">{summary.excusedDutyCount}</div>
                <div className="text-[10px] text-slate-500 mt-0.5">Post Night Guard Detail</div>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-tactical-900 border border-slate-800 text-center">
                ...
                <div className="text-[10px] text-slate-500 mt-0.5">MI Room / Sick Bay Cover</div>
              </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-tactical-950 border-b border-slate-800 text-slate-400 uppercase text-[10px] font-bold">
                  <tr>
                    ...
                    <th className="p-3.5">PT Attendance Status</th>
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan={6} className="p-12 text-center text-slate-500">
                        ...
                        <span>Loading daily PT sheet...</span>
                      </td>
                    </tr>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-5 rounded-2xl bg-tactical-900 border border-slate-800 text-center">
              <span className="text-xs text-slate-400 font-bold uppercase">7-Day PT Attendance</span>
              <div className="text-3xl font-black text-emerald-400 font-mono mt-1">{analytics.weeklyRate}%</div>
            </div>
            <div className="p-5 rounded-2xl bg-tactical-900 border border-slate-800 text-center">
              <span className="text-xs text-slate-400 font-bold uppercase">30-Day PT Attendance</span>
              <div className="text-3xl font-black text-blue-400 font-mono mt-1">{analytics.monthlyRate}%</div>
            </div>
            <div className="p-5 rounded-2xl bg-tactical-900 border border-slate-800 text-center">
              <span className="text-xs text-slate-400 font-bold uppercase">Yearly Cumulative PT Rate</span>
          ...
          <div className="rounded-2xl bg-tactical-900 border border-slate-800 p-5 space-y-3">
            <h4 className="text-sm font-bold text-white uppercase tracking-wider">Top 5 Consistent PT Performers (30-Day Rate)</h4>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
