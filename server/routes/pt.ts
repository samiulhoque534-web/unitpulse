...
// 1. Get Daily PT Sheet (Only PRESENT personnel available!)
ptRouter.get('/daily-sheet', authenticateUser, (req, res) => {
  ...
  const existingRecords = db.prepare('SELECT * FROM pt_records WHERE date = ?').all(date);
  ...
  existingRecords.forEach((rec: any) => {
    ...
    if (p && p.current_status !== 'PRESENT') {
      nonPresentWithRecords.push({
        ...
        remarks: rec.remarks,
        isEligible: false,
        warning: `Personnel is no longer eligible for PT (Status: ${p.current_status} - ${p.status_reason || 'Away'})`,
      });
    }
  });
...
// 2. Batch Save Daily PT Attendance
ptRouter.post('/daily-sheet', authenticateUser, requirePermission(['2IC', 'DUTY_OFFICER', 'DUTY_MUNSHI']), (req: AuthRequest, res) => {
  ...
  db.prepare('DELETE FROM pt_records WHERE date = ?').run(date);
  ...
  records.forEach((r: any) => {
    ...
    const id = `pt_${date}_${r.personnelId}`;
    db.prepare(`
  ...
  logAudit(req, 'SAVE_PT_ATTENDANCE', 'PT', undefined, `Saved PT attendance for ${savedCount} present soldiers on ${date}`);

  return res.json({ message: `Logged PT attendance for ${savedCount} personnel on ${date}.` });
});

// 3. PT Analytics (Weekly, Monthly, Yearly % calculated on eligible present sessions)
ptRouter.get('/analytics', authenticateUser, (req, res) => {
  ...
  const getUnitAttendanceRate = (days: number) => {
    ...
    const records = db.prepare('SELECT * FROM pt_records WHERE date >= ?').all(dStr);
  ...
  const personnelRankings = activePersonnel.map((p: any) => {
    ...
    const recs = db.prepare('SELECT * FROM pt_records WHERE personnel_id = ? AND date >= ?').all(p.id, dStr);
